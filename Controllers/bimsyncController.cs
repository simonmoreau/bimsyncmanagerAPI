...
namespace bimsyncManagerAPI.Controllers
{
    ...
    {
        string access_token;
        string folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), @"Downloaderbimsync\20180919");
        bool useProxy = true;
        ...
        {
            this.access_token = "9oZj1VB0vOpFnSDi95pBSo";
        ...
        public async Task GetProducts()
        {
            ...
            string projectId = "134c9142631f4b9c8cb905653f54e44b";
            ...
            using (StreamWriter logFile = new StreamWriter(logPath))
            {
                ...
                {
                    Dictionary<string, int> elementsBreakdown = await GetElementsBreakdown(projectId, revisionId);

                    foreach (KeyValuePair<string, int> values in elementsBreakdown)
                    {
                        await GetAProductsTypeForARevision(projectId, revisionId, values.Key, values.Value, logFile);
                    }
                }
            }
        }
        ...
        public async Task GetAProductsTypeForARevision(string projectId, string revisionId, string ifcType, int numberOfElements, StreamWriter logFile)
        {
            string outputPath = Path.Combine(folder, revisionId + "_" + ifcType + "_output.txt");
            string baseURL = @"https://api.bimsync.com/v2/projects/" + projectId;
            string url = baseURL + "/ifc/products?pageSize=1000&revision=" + revisionId + "&ifcType=" + ifcType;

            int pageNumber = Convert.ToInt32(Math.Round(Convert.ToDouble(numberOfElements/1000))) + 1; // await GetPageNumber(url);
            ...
            {
                string requestUrl = url + "&page=" + i.ToString();
            ...
            while (downloadIfcElementsTasks.Count > 0)
            {
                ...
                logFile.WriteLine(DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture) + "\t" + revisionId + "\t" + customTask.url);
        ...
        private async Task<Dictionary<string, int>> GetElementsBreakdown(string projectId, string revisionId)
        {
            string url = @"https://api.bimsync.com/v2/projects/" + projectId + "/ifc/products/ifctypes?revision=" + revisionId;
            ...
            using (HttpResponseMessage response = await GetResponseFromBimsync(url))
            {
                ...
                using (var jsonTextReader = new JsonTextReader(sr))
                {
                    object responseObject = serializer.Deserialize(jsonTextReader, typeof(object));

                    Dictionary<string, int> elementsBreakdown = new Dictionary<string, int>();
                    //Loop on elements Quantities
                    JObject elementsQuantities = responseObject as JObject;
                    if (elementsQuantities != null)
                    {
                        if (elementsQuantities.Children().Count() != 0)
                        {
                            foreach (JToken jToken in elementsQuantities.Children())
                            {
                                elementsBreakdown.Add(jToken.Path,jToken.Last.Value<int>());
                            }
                        }
                    }

                    return elementsBreakdown;
                }
            }
